      ...
      <div className='flex items-center justify-between font-medium'>
        ...
        <ul className={`hidden lg:flex flex-row text-black font-sans items-center gap-8 font-medium cursor-pointer m-4 duration-500 ${open ? "left-0" : "left-[100%]"}`}>
          {MenuItems.map((item, index) => (
            <li key={index} className="font-medium text-teal-200 hover:text-white"
                onMouseEnter={item.title === "Basketball" ? handleBasketballDropdown :
                             item.title === "Football" ? handleFootballDropdown :
                             item.title === "Soccer" ? handleSoccerDropdown :
                             item.title === "Formula 1" ? handleFormula1Dropdown : null}>
        ...
        {/* For mobile screens */}
        <ul ref={dropdownRef} className={`md:hidden z-20 top-0 mt-[60px] bg-white h-screen fixed w-full flex flex-col items-start py-5 transform transition-transform duration-500 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
          {MenuItems.map((item, index) => (
